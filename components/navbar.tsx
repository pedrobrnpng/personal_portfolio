        ...
        <div className="flex lg:hidden">
          <button className="transition-all" onClick={() => setIsOpen(!isOpen)}>
            <div className="relative h-10 w-10 bg-black focus:outline-none">
              ...
              <div className="absolute left-1/2 top-1/2 block w-5 -translate-x-1/2 -translate-y-2 transform">
                <span
                  aria-hidden="true"
                  className={`
          ...
          <ul className="flex">
            {links.map(({ name, url }, key) => {
              return (
                <li
                  key={key}
                  className="px-2 font-bold transition-all duration-200 hover:text-brown-beaver"
          ...
          <ul className="flex flex-col py-6 font-league text-2xl">
            {links.map(({ name, url }, key) => {
              return (
                <li
                  key={key}
                  className="bold px-2 transition-all duration-200 hover:text-brown-beaver"
                  onClick={() => setIsOpen(!open)}
                >
                  <Link href={url}>{name}</Link>
                </li>
