...
export default function Navbar() {
  ...
  const [wHeight, setHeight] = useState<number>(0);
  ...
  const transition = {
    type: 'spring',
    mass: 0.7,
    damping: wWidth < 1024 ? 13 : 10,
    delay: wWidth < 1024 ? 0 : 0.5,
  };
    ...
    >
      ...
      {isOpen && (
        <motion.div
          ...
          className="flex h-screen w-screen flex-col items-center justify-center bg-dark-purple lg:hidden"
        >
          <ul className="flex flex-col py-6 text-center font-league text-2xl lg:text-left">
            {links.map(({ name, url }, key) => {
              return (
                <li
                  key={key}
